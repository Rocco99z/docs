...
const ProTable: React.FC<ProTableProps> = (props: ProTableProps) => {
  const { index = false, columns: propsColumns, action, toolBar, ...rest } = props
        ...
        render: (text: string, record: any, index: number) => {
          return (
            <div
              style={{
                borderRadius: "50%",
                backgroundColor: `rgba(0, 0, 0,${1 - index / 10})`,
                width: "18px",
                height: "18px",
                color: "#fff",
                textAlign: "center",
  ...
  return (
    <div>
      {toolBar && (
        <div className='toolbar'>
          ...
          >
            {toolBar?.importProps && (
              <Upload {...importProps}>
                <Button icon={<ExportOutlined />} loading={importLoading} disabled={importLoading}>
                  导入
                </Button>
              </Upload>
            )}
            {toolBar?.exportProps && (
              <Button onClick={handleExport} loading={exportLoading} icon={<ImportOutlined />}>
                导出
              </Button>
            )}
      ...
      <Table rowKey={"key"} columns={columns} {...rest} />
    </div>
  )
}
